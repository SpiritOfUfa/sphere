...
describe("Autorization Form", () => {
  it("Check the header name and name of button  ", async () => {
    ...
    expect(await LoginClass.voytiButtonTitle).to.equal(
      LoginClass.expectedVoytiButtonTittle
    );
  ...
  it("Check the name of placeholders(Почта, Пароль)", async () => {
    expect(await LoginClass.emailPlaceholder).to.equal(
      LoginClass.expectedEmailPlaceholder
    );
    expect(await LoginClass.passwordPlaceholder).to.equal(
      LoginClass.expectedPasswordPlaceholder
    );
  });

  it("Check the button name", async () => {
    expect(await LoginClass.submitButtonName).to.equal(
      LoginClass.expectSubmitButtonName
    );
  });
  ...
  it("Validate Login form with incorrect password or email", async () => {
    ...
    expect(await LoginClass.hideNotification.getAttribute("class")).to.equal(
      LoginClass.unActiveNotificationAttribute
    );
    await LoginClass.submitFormButton.click();
    await browser.pause(550);
    expect(await LoginClass.showNotification.getAttribute("class")).to.equal(
      LoginClass.activeNotificationAttribute
    );
    expect(await LoginClass.notificationText).to.equal(
      LoginClass.expectedNotificationText
    );
    LoginClass.showPasswordBut.click();
    await browser.pause(5000);
    expect(await LoginClass.showPasswordButAttribute).to.equal(
      LoginClass.incorrectPasswordInput
    );
  });
